...
export default function LedgerScreenRoute() {
  ...
  const entries = useMemo(() => mobileTransactions.filter((entry) => (type === 'all' || entry.type === type) && [entry.merchant, entry.category].join(' ').toLowerCase().includes(query.toLowerCase())), [query, type]);
  return <LedgerScreen><View><Text style={styles.eyebrow}>TRANSACTIONS</Text><Text style={styles.title}>Your financial record.</Text><Text style={styles.copy}>Search and review every event in the ledger.</Text></View><GlassSurface><TextInput accessibilityLabel="Search entries" onChangeText={setQuery} placeholder="Search entries" placeholderTextColor={Colors.light.textSecondary} style={styles.search} value={query} /><View style={styles.filters}>{(['all', 'expense', 'income'] as const).map((value) => <Pressable key={value} onPress={() => setType(value)} style={[styles.filter, type === value && styles.filterActive]}><Text style={[styles.filterText, type === value && styles.filterTextActive]}>{value === 'all' ? 'All' : value === 'expense' ? 'Expenses' : 'Income'}</Text></Pressable>)}</View><View style={styles.list}>{entries.map((item) => <View key={item.id} style={styles.entry}><View style={[styles.entryIcon, { backgroundColor: item.type === 'income' ? Colors.light.successSoft : Colors.light.dangerSoft }]}><Text style={{ color: item.type === 'income' ? Colors.light.success : Colors.light.danger }}>{item.type === 'income' ? '↑' : '↓'}</Text></View><View style={styles.entryText}><Text style={styles.entryTitle}>{item.merchant}</Text><Text style={styles.entryMeta}>{item.category} · {item.date}</Text></View><View style={styles.amountColumn}>{item.status === 'review' ? <Text style={styles.review}>Review</Text> : null}<Text style={[styles.amount, { color: item.type === 'income' ? Colors.light.success : Colors.light.text }]}>{item.type === 'income' ? '+' : '−'}{currency(item.amount)}</Text></View></View>)}</View></GlassSurface></LedgerScreen>;
}
...
const styles = StyleSheet.create({
  ...
  search: { backgroundColor: Colors.light.background, borderColor: Colors.light.border, borderWidth: 1, borderRadius: 8, color: Colors.light.text, fontSize: 14, minHeight: 44, paddingHorizontal: 12 },
  ...
  filterText: { color: Colors.light.textSecondary, fontSize: 12, fontWeight: '700' },

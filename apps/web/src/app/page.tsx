...
export default function OverviewPage() {
  ...
  const maxExpense = Math.max(
    ...(breakdown?.expenses.map((item) => item.amount) ?? [1]),
  );
      ...
      <section className="grid gap-6 lg:grid-cols-5">
        <div className="surface-card p-5 sm:p-6 lg:col-span-3">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold text-ink">Net history</h2>
              ...
              history.map((item) => (
                ...
                >
                  <div
                    className={
                      item.net >= 0
                        ? "w-full rounded-t-md bg-success/80"
                        : "w-full rounded-t-md bg-danger/80"
                    }
                    style={{
                      height: `${Math.max(16, (Math.abs(item.net) / maxNet) * 100)}%`,
                    }}
                  />
...
function Metric({
  ...
  label: string;
  value: string;
  icon: "trend-up" | "trend-down" | "wallet" | "chart";
  tone: "success" | "danger" | "action";
}) {
  const className =
    tone === "success"

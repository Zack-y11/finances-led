...
export function GroupsView() {
  ...
  async function load() {
    ...
    setError(undefined);
    try {
      setGroups(await getEntryGroups());
    } catch (reason) {
      setError(
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!parsed.success) {
      setError(parsed.error.issues[0]?.message ?? "Enter valid group details.");
      return;
    }
    setSaving(true);
    try {
      await createEntryGroup(parsed.data);
      setCreating(false);
      setNotice("Group created. You can now add ledger entries to it.");
      await load();
    } catch (reason) {
      setError(
        ...
        >
          <div className="sm:col-span-2">
            ...
            <p className="mt-1 text-sm text-muted">
              Groups are saved immediately and can receive ledger entries.
            </p>
          </div>
          <label>
            Name
          ...
          {groups.map((group) => (
            ...
            >
              ...
              <div className="mt-6 flex items-end justify-between border-t border-border pt-4">
                <span className="text-xs text-muted">View group activity</span>

      ...
      <section>
        ...
        <div className="grid gap-3">
          {entries.map((entry) => {
            ...
            return <div key={entry.id} className="grid grid-cols-[1fr_1fr_auto] gap-3 border-b border-zinc-200 py-2"><span>{entry.merchant ?? "—"}</span><span className="text-zinc-600">{entry.group?.name ?? entry.category?.name ?? "Uncategorized"}</span><span>{sign}${Number(entry.amount).toFixed(2)}</span></div>;
          })}

...
export function CreateEntryForm({ accounts, categories }: { accounts: Option[]; categories: Option[] }) {
  ...
  return (
    <form onSubmit={submit} className="grid gap-4 rounded border border-zinc-200 p-5">
      <label>Type<select name="type" className={nativeSelectClassName}><option value="expense">Expense</option><option value="income">Income</option><option value="adjustment">Adjustment</option></select></label>
      <label>Amount<Input name="amount" type="number" min="0.01" step="0.01" placeholder="3.19" required /></label>
      <label>Merchant<Input name="merchant" placeholder="Starbucks" required /></label>
      <label>Account<select name="accountId" className={nativeSelectClassName} required>{accounts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label>Category<select name="categoryId" className={nativeSelectClassName} required>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
      <label>Date<Input name="date" type="date" defaultValue={new Date().toISOString().slice(0, 10)} required /></label>
      <label>Note<Input name="note" placeholder="Coffee" /></label>
      {error && <p className="text-sm text-red-700">{error}</p>}
      <Button disabled={saving || !accounts.length || !categories.length}>{saving ? "Saving…" : "Save entry"}</Button>
    </form>
  );
}
